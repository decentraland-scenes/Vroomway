...
import { type WorldMoveVehicleState } from './world/worldMoveVehicle'
import { type ScenePhysicsConf, type SceneSkyboxConf } from './sceneConfigType'
import { type InitAvatarTrap } from './modules/avatarTrap/avatarTrap'
import { type AvatarData } from './modules/avatar/avatarSwap'
import { type GridDebugType } from './terrain/terrainGrid'
...
export class Config {
  ...
  WORLD_VEHICLE_DEFAULTS: WorldMoveVehicleState = {
    ...
    mass: 5,
    shapeRadius: 0.5, // how big the sphere is
    linearDamping: 0.4,
    angularDamping: 0.4,
    // stat stuff
    gasActive: false,
  ...
  initForEnv(): void {
    ...
    this.infinEngineCenter = Vector3.create(
      this.sizeX / 2,
      this.sizeY / 2 - 10,
      this.sizeZ / 2
    )
    ...
    this.parcelSize = Vector3.create(
      this.sizeX / PARCEL_UNIT_METERS,
      this.sizeY / PARCEL_UNIT_METERS,
      this.sizeZ / PARCEL_UNIT_METERS
    )
  }
}
