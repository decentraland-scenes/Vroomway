...
import type IGridEntity from './IGridEntity'
...
EMPTY_ARRAY.push = (entity: any) => {
  debugger
  throw new Error('EMPTY_SET is read only')
}
EMPTY_ARRAY.splice = (entity: any) => {
  throw new Error('EMPTY_SET is read only')
}
EMPTY_ARRAY.indexOf = (entity: any) => {
  return -1
}
...
export default class Grid {
  ...
  map = new Map<number, IGridEntity[]>()
   ...
   * @param {Object} entity
   */
  addEntity(entity: IGridEntity): void {
    const index = this.getGridIndex(entity.x, entity.y, entity.z) | 0
    entity.setGrid(this)
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    this.map.has(index)
      ? this.map.get(index)!.push(entity)
      : this.map.set(index, [entity])
   ...
   * @param {Object} entity
   */
  removeEntity(entity: IGridEntity): void {
    const index = this.getGridIndex(entity.x, entity.y, entity.z) | 0
  ...
  moveEntity(
    ...
    this.map.has(newIndex)
      ? this.map.get(newIndex)!.push(entity)
      : this.map.set(newIndex, [entity])
  }
