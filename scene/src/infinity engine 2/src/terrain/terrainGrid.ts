...
import {
  GridEntityMgr,
  type GridEntityMgrArgs,
  GridTypeEnum
} from '../modules/spacePartition/IGridEntity'
import { REGISTRY } from '../registry'
import { type TileSetConf } from '../sceneConfigType'
import { MovesWithWorld } from '../world/worldMoveComponent'

const CLASSNAME = 'terrainGrid.ts'

export function destroyTerrainGrid(): void {
...
// terrain factory
export function createTerrainGrid(args: GridCreateRequest): void {
  log('CONFIG', CONFIG)
  ...
  console.log(
    CLASSNAME,
    'calling create REGISTRY.spacePartioner',
    REGISTRY.spacePartioner
  )
  ...
  if (CONFIG.ENABLE_GRID_SYSTEM) {
    const moveWithWorldGrid = createGrid(args)
...
export function getGridDims(tileset: TileSetConf): TilesetGridDims {
  ...
  const cellYRatio =
    terrainTileSet.tile_dimensions[TILE_SET_Y_IDX] /
    terrainTileSet.tile_dimensions[TILE_SET_X_IDX]
  ...
  const cellOffset = Vector3.create(
    CENTER_OFFSET,
    GRID_CELL_SIZE_Y / 2,
    CENTER_OFFSET
  )
...
function createGrid(args: GridCreateRequest): GridCreateResult {
  // const y = 0
  ...
  const generalOffset = args.tileset.tilesetOffset
    ? args.tileset.tilesetOffset
    : Vector3.Zero()
  // const cellOffset = args.cellOffset
  // const GRID_WIDTH_X = args.dimX
  ...
  for (let x = 0; x <= GRID_WIDTH_X; x++) {
    ...
    for (let y = 0; y <= GRID_WIDTH_Y; y++) {
      for (let z = 0; z <= GRID_WIDTH_Z; z++) {
        ...
        const gridIdx = REGISTRY.spacePartioner.getGridIndex(
          position.x,
          position.y,
          position.z
        )
        ...
        const tileCell =
          terrainTileSet.tiles &&
          terrainTileSet.tiles[TILE_SET_X] &&
          terrainTileSet.tiles[TILE_SET_X][TILE_SET_Y] &&
          terrainTileSet.tiles[TILE_SET_X][TILE_SET_Y][TILE_SET_Z]
            ? terrainTileSet.tiles[TILE_SET_X][TILE_SET_Y][TILE_SET_Z]
            : undefined
        ...
        const debugText =
          ...
          ')\n' +
          'src:' +
          (y == _yGridIdxTracker ? (tileCell ? tileCell.src : '') : '----')

        const hasModel = tileCell?.src && y == _yGridIdxTracker
        ...
        const showFn = () => {
          ...
          if (args.debug.enabled) {
            Material.setPbrMaterial(
              cellEntities.visibleDebugCell,
              args.debug.cellMaterial
            )
            MeshRenderer.setBox(cellEntities.visibleDebugCell)

            // TODO let config set color and font size
            TextShape.createOrReplace(cellEntities.debugText, {
              text: debugText,
              textColor: Color4.Blue(),
              fontSize: 4
            })
            Billboard.createOrReplace(cellEntities.debugText, {
              billboardMode:
                BillboardMode.BM_Y | BillboardMode.BM_Z | BillboardMode.BM_X
            })
          }
          if (hasModel) {
            GltfContainer.createOrReplace(cellEntities.visible, {
              src:
                terrainTileSetModelFolder + tileCell.src + '.' + tileModelExt,
