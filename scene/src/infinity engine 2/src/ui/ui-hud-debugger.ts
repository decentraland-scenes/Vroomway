...
function updateDebugButtonUI(testButton: PromptButton): void {
  if (changeButtonWidth > 0)
    testButton.imageElement.uiTransform!.width = changeButtonWidth
  if (changeButtonHeight > 0)
    testButton.imageElement.uiTransform!.height = changeButtonHeight
  testButton.labelElement.fontSize = 14
}
// eslint-disable-next-line @typescript-eslint/explicit-function-return-type, @typescript-eslint/no-unused-vars
function boolShortNameOnOff(val: boolean): string {
  if (val) return 'On'
  return 'Off'
}
let alreadyInit = false
export async function createDebugUIButtons(): Promise<void> {
  ...
  const testControlsToggle = ui.createComponent(ui.CustomPrompt, {
  ...
  const testControls = ui.createComponent(ui.CustomPrompt, {
    style: ui.PromptStyles.DARKLARGE,
    width: 1,
    height: 1,
    startHidden: false
