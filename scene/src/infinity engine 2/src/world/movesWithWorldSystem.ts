...
export function destroyMovesWithWorldSystem(): void {
  if (movesWithWorldSystem) movesWithWorldSystem.removeFromEngine()
}
export function initMovesWithWorldSystem(): void {
  // MOVE WITH WORLD SYSTEM
  const movesWithWorldSystemFn = (dt: number): void => {
  ...
  movesWithWorldSystem = new SystemWrapperBasic(
    {
      name: 'movesWithWorldSystemFn',
      priority: SYSTEM_PRIORITES.REGULAR,
      fn: movesWithWorldSystemFn
    },
    movesWithWorldState
  )
