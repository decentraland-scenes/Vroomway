...
import { type BaseAvatar } from '../modules/avatar/avatarSwap'
...
export function initWorldMove(sceneConf: SceneConfig): void {
  ...
  // debug markers
  const debugMarkers = new DebugSpherePool()
  ...
  function physicsSystem(dt: number): void {
    ...
    if (vehicle) {
      ...
      Vector3.copyFrom(
        vehicle.physicsBody.position,
        Transform.getMutable(vehicle.entity).position
      )
      Transform.getMutable(vehicle.entity).rotation =
        vehicle.physicsBody.quaternion
...
export function initWorldState(sceneConf: SceneConfig, car: BaseAvatar): void {
