...
import { type ColliderCreateResult } from '../terrain/terrainColliders'
...
export function createWorldMoveVehicleContacts(
  result: ColliderCreateResult
): void {
  ...
  for (const collider of result.colliders) {
    ...
    const colliderontactMaterial: CANNON.ContactMaterial =
      new CANNON.ContactMaterial(playerMaterial, collider.body.material, {
...
export function initWorldMoveVehicle(sceneConfig: SceneConfig): void {
  ...
  const vehicle: WorldMoveVehicle = createWorldMoveVehicle(
...
export function createWorldMoveVehicle(
  position: Vector3,
  cannonWorld: CANNON.World,
  worldVehicleConf: WorldMoveVehicleState
): WorldMoveVehicle {
  const ball = engine.addEntity()
  let body: CANNON.Body
  const world = cannonWorld
  ...
  if (false) {
    ...
    const chassisShape = new CANNON.Box(
      new CANNON.Vec3(2 / scale, 0.5 / scale, 1 / scale)
    )
    ...
    vehicle.wheelInfos.forEach((wheel) => {
      ...
      if (false) {
        ...
        const q: CANNON.Quaternion = new CANNON.Quaternion()
