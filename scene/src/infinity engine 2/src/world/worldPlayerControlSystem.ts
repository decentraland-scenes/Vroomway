...
import { type WorldState } from './worldMoveState'
...
export function initPlayerControlSystemSystem(worldState: WorldState): void {
  ...
  const playerControlSystemFn = (dt: number) => {
    ...
    worldVehicleState.brakesActive = inputSystem.isPressed(
    ...
    if (
      inputSystem.isTriggered(InputAction.IA_FORWARD, PointerEventType.PET_UP)
    ) {
      worldVehicleState.gasActive = false
    }
    if (
      inputSystem.isTriggered(InputAction.IA_BACKWARD, PointerEventType.PET_UP)
    ) {
    ...
    if (camTransform) {
      ...
      worldMoveVehicle.physicsBody.applyForce(
        new CANNON.Vec3(camHorizontal.x, 0, camHorizontal.z),
        worldMoveVehicle.physicsBody.position
      )
    }
    ...
    if (worldVehicleState.jump) {
      ...
      worldMoveVehicle.physicsBody.applyImpulse(
        impuse,
        worldMoveVehicle.physicsBody.position
      )
    ...
    if (worldVehicleState.spin) {
      ...
      if (avatarTrap)
        Transform.getMutable(avatarTrap.entity).rotation = Quaternion.multiply(
          Transform.get(avatarTrap.entity).rotation,
          Quaternion.fromEulerDegrees(0, 400 * dt, 0)
        )
      // from state!
      if (carTransform)
        carTransform.rotation = Quaternion.multiply(
      ...
      if (
        ...
        (Math.abs(worldMoveVehicle.physicsBody.velocity.x) > 0.01 ||
          Math.abs(worldMoveVehicle.physicsBody.velocity.y) > 0.01 ||
          Math.abs(worldMoveVehicle.physicsBody.velocity.z) > 0.01)
      ) {
        carTransform.rotation = Quaternion.slerp(
          carTransform.rotation,
          Quaternion.lookRotation(
            worldMoveVehicle.physicsBody.velocity,
            VECTOR_UP
          ),
          0.6
        )
    ...
    if (logInterval.update(dt)) {
      log(
        CLASSNAME,
        'playerControlSystemFn',
        'speed',
        worldVehicleState.speed,
        'gasActive',
        worldVehicleState.gasActive,
        'brakesActive',
        worldVehicleState.brakesActive,
        'spin',
        worldVehicleState.spin,
        'jump',
        worldVehicleState.jump,
        'physics.velocity',
        worldMoveVehicle.physicsBody.velocity
    ...
    if (CONFIG.SHOW_GAME_DEBUG_INFO && camHorizontal) {
      const debugT = Transform.getMutable(worldState.debugEntity)
    ...
    if (
      worldMoveVehicle.physicsBody.position.y < worldState.respawnIfBelowPosY
    ) {
      log(
        CLASSNAME,
        'playerControlSystemFn',
        'respawned player',
        'respawnIfBelowPosY',
        REGISTRY.worldState.respawnIfBelowPosY,
        'respsawnPosY',
        REGISTRY.worldState.respsawnPosY
      )
  ...
  playerControlSystem = new SystemWrapperBasic(
    {
      name: 'playerControlSystemFn',
      priority: SYSTEM_PRIORITES.REGULAR,
      fn: playerControlSystemFn
    },
    playerControlState
  )
