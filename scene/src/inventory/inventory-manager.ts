...
import { CargoObjectData, LevelingObjectData, PowerUpObjectData, ResourceObjectData, TokenObjectData } from "./inventory-data";
...
export class InventoryEntry {
    ...
    public get IsModified():boolean { return this.isModified };
...
export class InventoryManager {
    ...
    public get ItemTypes():number { return this.itemRegistryViaType.length; }
    // returns length of registry of given type
    public GetRegistryLengthOfType(type: number): number { return this.itemRegistryViaType[type].size(); }
    ...
    public constructor() {
        ...
        for (let i: number = 0; i < LevelingObjectData.length; i++) {
            const entry: InventoryEntry = new InventoryEntry(i, LevelingObjectData[i].ID);
        ...
        for (let i: number = 0; i < ResourceObjectData.length; i++) {
            const entry: InventoryEntry = new InventoryEntry(i, ResourceObjectData[i].ID);
            this.itemRegistryViaType[1].addItem(i.toString(), entry);
            this.itemRegistryViaID.addItem(entry.ID, entry);
            // push to rarity
            //  ensure rarity list exists
            if (!this.itemRegistryViaRarity.containsKey(ResourceObjectData[i].Rarity.toString()))
                this.itemRegistryViaRarity.addItem(ResourceObjectData[i].Rarity.toString(), new List<InventoryEntry>());
            //  add entry to rarity listing
            this.itemRegistryViaRarity.getItem(ResourceObjectData[i].Rarity.toString()).addItem(entry);
        }
        //  cargo
        ...
        for (let i: number = 0; i < PowerUpObjectData.length; i++) {
            const entry: InventoryEntry = new InventoryEntry(i, PowerUpObjectData[i].ID);
    ...
    public CallbackResetItems(type: number):void { InventoryManager.Instance.ResetItems(type); }
    public ResetItems(type: number):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: reset item");
    ...
    public IsItemModifiedByIndex(type: number, index: string): boolean {
        if (InventoryManager.IsDebugging) console.log("Inventory System: is item modified");
    ...
    public CallbackSetItemCountByIndex(type: number, index: string, value: number, reset: boolean = false):void { InventoryManager.Instance.SetItemCountByIndex(type, index, value, reset); }
    public SetItemCountByIndex(type: number, index: string, value: number, reset: boolean = false):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: reset item");
    ...
    public CallbackAddItemCountByIndex(type: number, index: string, value: number):void { InventoryManager.Instance.AddItemCountByIndex(type, index, value); }
    public AddItemCountByIndex(type: number, index: string, value: number):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: adding count to item, type=" + type + ", index=" + index + ", value=" + value);
        this.itemRegistryViaType[type].getItem(index).Amount += value;
    }

    //  updates the ui update meathod of the target item 
    public CallbackSetUpdateUI(type: number, index: string, callbackUpdateUI: () => void):void { InventoryManager.Instance.RegisterUpdateUI(type, index, callbackUpdateUI); }
    public RegisterUpdateUI(type: number, index: string, callbackUpdateUI: () => void):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: adding count to item, type=" + type + ", index=" + index);
        this.itemRegistryViaType[type].getItem(index).RegisterItemUICallback(callbackUpdateUI);
    }
    ...
    public IsItemModifiedByType(type: number, index: string): boolean {
        if (InventoryManager.IsDebugging) console.log("Inventory System: adding count to item by type|pos, type=" + type + ", index=" + index);
        return this.itemRegistryViaType[type].getItem(index).IsModified;
    }
    ...
    public GetItemCountByType(type: number, index: string, reset: boolean = false): number {
        if (InventoryManager.IsDebugging) console.log("Inventory System: getting count of item by type|pos, type=" + type + ", index=" + index);
        if (reset) this.itemRegistryViaType[type].getItem(index).IsModified = false;
        return this.itemRegistryViaType[type].getItem(index).Amount;
    }

    //  set current count of target item 
    public CallbackSetItemCountByType(type: number, index: string, value: number, reset: boolean = false):void { InventoryManager.Instance.SetItemCountByType(type, index, value, reset); }
    public SetItemCountByType(type: number, index: string, value: number, reset: boolean = false):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: setting count of item by type|pos, type=" + type + ", index=" + index + ", value=" + value);
        this.itemRegistryViaType[type].getItem(index).Amount = value;
        if (reset) this.itemRegistryViaType[type].getItem(index).IsModified = false;
    }

    //  add given amount to target item
    public CallbackAddItemCountByType(type: number, index: string, value: number):void { InventoryManager.Instance.AddItemCountByType(type, index, value); }
    public AddItemCountByType(type: number, index: string, value: number):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: adding count to item by type|pos, type=" + type + ", index=" + index + ", value=" + value);
        this.itemRegistryViaType[type].getItem(index).Amount += value;
    }
    ...
    public GetItemCountByID(id: any, reset: boolean = false): number {
        if (InventoryManager.IsDebugging) console.log("Inventory System: getting count of item by id, id=" + id);
    ...
    public CallbackSetItemCountByID(id: any, value: number, reset: boolean = false):void { InventoryManager.Instance.SetItemCountByID(id, value, reset); }
    public SetItemCountByID(id: any, value: number, reset: boolean = false):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: setting count of item by id, id=" + id + ", value=" + value);
        this.itemRegistryViaID.getItem(id.toString()).Amount = value;
        if (reset) this.itemRegistryViaID.getItem(id.toString()).IsModified = false;
    }

    //  modify item count using item's name as indexing 
    public CallbackAddItemCountByID(id: any, value: number):void { InventoryManager.Instance.AddItemCountByID(id, value); }
    public AddItemCountByID(id: any, value: number):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: adding count to item by id, id=" + id + ", value=" + value);
        this.itemRegistryViaID.getItem(id.toString()).Amount += value;
    }

    //  updates the ui update meathod of the target item 
    public RegisterUpdateUIByID(id: any, callbackUpdateUI: () => void):void {
        if (InventoryManager.IsDebugging) console.log("Inventory System: registering callback to item id=" + id);
    ...
    public GetRandomResourceViaRarity(rarity: number): string {
        if (InventoryManager.IsDebugging) console.log("Inventory System: getting random resource of rarity=" + rarity);
        const rarityListing: List<InventoryEntry> = this.itemRegistryViaRarity.getItem(rarity.toString());
        return rarityListing.getItem(Math.floor(Math.random() * rarityListing.size())).ID;
    }
}